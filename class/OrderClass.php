...
class OrderClass
{
	...
	public static function find_orders_users()
	{
		...
		if ($object->charge == 0.00)
						{
							$charge = "<a href='index.php?content=price&order_id={$object->order_id}&user_id={$object->user_id}'>".$object->charge."</a>";
						}
						else
						{
	...
	public static function update_charge_by_id($order_id, $charge)
	{
		...
		$database->fire_query($query);
		echo "De prijs van de order is succesvol veranderd.";
